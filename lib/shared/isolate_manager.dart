...
class IsolateManager {
  ...
  Future<void> initializeIsolate() async {
    ...
    _dataQueue.stream.listen((data) {
      print("[IsolateManager, listening to dataQueue] oh hey new data: ${data.toString()}");
    ...
    receivePort.listen((message) async {
      ...
      if (data is ({Question question, Answer? answer})) {
        if (data.answer != null) {  // Interação com o chat
          ...
          data.answer?.rating = Utilities.extractRatingFromCorrection(input: answer);

          sendPort.send({'id': id, 'data': {'answer': data.answer!, 'aaUID': data.question.answerAggregatorUID!}});
        } else {
          // TODO
